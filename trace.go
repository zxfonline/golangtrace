...
import (
	...
	"github.com/zxfonline/golangtrace/timeseries"
)
...
func GetFamilyDetailString(family string, bucket int) string {
	if bucket >= 0 && bucket <= bucketsPerFamily {
		if b := lookupBucket(family, bucket); b != nil {
			traces := b.Copy(false)
			if len(traces) > 0 {
				if byteData, err := json.Marshal(traces); err == nil {
...
func (tr *trace) GetFamily() string {
	return tr.Family
}

func (tr *trace) GetElapsedTime() int64 {
	return tr.Elapsed.Nanoseconds()
}

const (
	bucketsPerFamily    = 9
	tracesPerBucket     = 100
	maxActiveTraces     = 200 // Maximum number of active traces to show.
